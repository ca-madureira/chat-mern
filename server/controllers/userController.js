...
const registerUser = async (req, res) => {
  try {
    ...
    if (user) return res.status(400).json("Usuario com este email ja existe ");

    if (!name || !email || !password)
      return res.status(400).json("Todos os campos são obrigatorios");

    if (!validator.isEmail(email))
      return res.status(400).json("Email deve ser valido");

    if (!validator.isStrongPassword(password))
      return res.status(400).json("senha deve ser forte");
